...
安装脚本会自动：
- ✅ 检查并安装 **pyenv**（Python版本管理工具）
- ✅ 检查并安装 **pipenv**（虚拟环境和依赖管理工具）
- ✅ 使用 pyenv 安装并切换指定的 Python 版本（当前要求：**Python 3.9**）
- ✅ 使用 pipenv 创建虚拟环境并安装依赖（**自动锁定版本**）
- ✅ 验证安装是否成功

### 环境管理：pyenv + pipenv

本项目使用 **pyenv** 和 **pipenv** 来确保环境一致性：
...
- **Windows:**
  ...
  # 或使用 Chocolatey: choco install pyenv-win
  ```

项目使用 `.python-version` 文件指定 Python 版本。安装脚本会自动：
- 检查 pyenv 是否安装
- 如果未安装指定版本，自动安装
- 设置本地 Python 版本
...
**pipenv 的优势：**
- ✅ **自动虚拟环境管理** - 无需手动创建和激活
- ✅ **依赖锁定** - `Pipfile.lock` 确保所有环境安装相同版本
- ✅ **开发依赖分离** - 区分生产依赖和开发依赖
- ✅ **安全性** - 自动检查依赖漏洞
...
# 运行命令（自动使用虚拟环境）
pipenv run python coldstart.py [目标项目目录]
...
- ✅ **pyenv + pipenv** - 专业的 Python 版本和依赖管理，确保环境完全一致
- ✅ **Jinja2模板引擎** - 专业的模板处理
- ✅ **三阶段流程** - 初始化 → 处理 → 导出
- ✅ **组件化规则** - 根据项目配置自动选择规则
- ✅ **配置驱动** - 所有选项通过配置文件管理
...
│   ├── commands.py             # 交互式命令系统
│   └── main.py                # 主入口函数
├── start                       # 启动脚本（Mac/Linux，使用 pipenv）
├── start.bat                   # 启动脚本（Windows，使用 pipenv）
├── install.py                  # 依赖安装脚本（Python，跨平台）
├── install.sh                  # 依赖安装脚本（Mac/Linux）
├── install.bat                 # 依赖安装脚本（Windows）
├── Pipfile                     # pipenv 依赖配置文件
├── Pipfile.lock                # pipenv 依赖锁定文件（自动生成）
├── .python-version             # pyenv Python 版本文件
├── .venv/                      # pipenv 虚拟环境（自动创建，已忽略）
├── project-init/
│   ├── templates/              # Jinja2模板
│   │   ├── plans/              # 计划模板
│   │   ├── rules/              # 规则模板
...
- **Python 3.9** - 主编程语言（使用 pyenv 管理版本）
- **pipenv** - 虚拟环境和依赖管理
- **Jinja2** - 模板引擎
- **JSON** - 配置文件格式
