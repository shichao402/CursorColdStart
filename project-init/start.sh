...
# 步骤1：复制初始化系统文件
echo -e "${GREEN}[步骤 1/5] 复制初始化系统文件...${NC}"
...
# 步骤2：创建项目计划文档（在冷启动目录内）
echo -e "${GREEN}[步骤 2/5] 创建项目计划文档...${NC}"
# 确保冷启动目录存在
mkdir -p "$INIT_DIR"
PROJECT_PLAN="$INIT_DIR/ProjectPlan.md"

if [ -f "$PROJECT_PLAN" ]; then
    echo -e "${YELLOW}⚠️  文件已存在: $PROJECT_PLAN${NC}"
    read -p "是否覆盖？(y/n) " -n 1 -r
    echo ""
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        echo -e "${YELLOW}跳过创建 ProjectPlan.md${NC}"
    else
        cp "$SCRIPT_DIR/ProjectPlan.md.template" "$PROJECT_PLAN"
        echo -e "${GREEN}✅ 项目计划模板已复制${NC}"
        echo -e "${YELLOW}📝 请编辑 $PROJECT_PLAN 填写项目信息${NC}"
    fi
else
    cp "$SCRIPT_DIR/ProjectPlan.md.template" "$PROJECT_PLAN"
    echo -e "${GREEN}✅ 项目计划模板已创建: $PROJECT_PLAN${NC}"
    echo -e "${YELLOW}📝 请编辑 $PROJECT_PLAN 填写项目信息${NC}"
fi
echo ""

# 步骤3：设置规则文件
echo -e "${GREEN}[步骤 3/5] 设置AI助手规则文件...${NC}"
...
if [ ! -f "$INIT_DIR/ProjectPlan.md" ]; then
    echo -e "${RED}❌ 项目计划文档不存在${NC}"
    ERRORS=$((ERRORS + 1))
else
    echo -e "${GREEN}✅ 项目计划文档存在${NC}"
fi
...
if [ $ERRORS -eq 0 ]; then
    ...
    echo ""
    echo "1. 📝 编辑项目计划文档："
    echo "   $INIT_DIR/ProjectPlan.md"
    echo ""
    echo "2. 🤖 在 Cursor 中告诉 AI 助手："
    echo "   请按照 PROJECT_INIT_PLAN.md 开始项目初始化"
    echo ""
    echo "   或者简单说："
    echo "   开始项目初始化"
    echo ""
    echo "3. 💬 AI 助手会引导你完成后续步骤"
    echo ""
    echo -e "${YELLOW}提示：${NC}"
    echo "- 确保已填写 $INIT_DIR/ProjectPlan.md 中的基本信息"
    echo "- AI 助手会询问技术选型细节，按提示回答即可"
